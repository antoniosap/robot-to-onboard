...
class BaseOnBoard(Node):
    ...
    def __init__(self):
        ...
        # pub
        self.pub_display8x8 = self.create_publisher(String, '/sensehat/led_panel', qos)
        self.pub_stick = self.create_publisher(String, '/base/stick', qos)
        self.pub_cam_light_led = self.create_publisher(Bool, '/base/cam_light_led', qos)
        self.pub_cam_pan = self.create_publisher(Int16, 'base/cam_pan', qos)
        self.pub_cam_tilt = self.create_publisher(Int16, 'base/cam_tilt', qos)
        #
        time.sleep(1)  # wait processes spinning and board card started
        self.home_on()
        # sub - Don't subscribe until everything has been initialized.
        self.btn_shutdown_sub = self.create_subscription(Bool, "/base/btn_shutdown", self.btn_shutdown_cb, qos)
        self.btn_stick_sub = self.create_subscription(String, "/sensehat/stick", self.btn_stick_cb, qos)
    ...
    def btn_shutdown_cb(self, data):
        self.logger.debug(f'shutdown button {data}')
    ...
    async def asy_axis_spin(self):
        try:
            self.logger.info(f'{self.node_name} Starting: asy_axis_spin')
            time.sleep(10)   # wait processes spinning and board card 'base_ardu' started
            self.logger.info(f'{self.node_name} Started: asy_axis_spin')
            rate = self.create_rate(self.spin_hertz)  # hz
            while True:
                if self.axis_move_exec:
                    self.logger.info(f'{self.node_name} -----> 1')
                    await self.two_axis.asy_exec_trajectory()
                    self.logger.info(f'{self.node_name} -----> 2 {self.two_axis.axis1.q1} {self.two_axis.axis2.q2}')
                    self.cam_cur = (self.two_axis.axis1.q1, self.two_axis.axis2.q1)
                    self.logger.info(f'{self.node_name} -----> 3')
                    self.axis_move_exec = False
                rate.sleep()
        except Exception as error:
            self.logger.error(f'{base_rpi.node_name} asy_axis_spin: {error}')

    async def asy_ros_spin(self):
        try:
            self.logger.info(f'{self.node_name} Starting: asy_ros_spin')
            time.sleep(5)
            self.logger.info(f'{self.node_name} Started: asy_ros_spin')
            rate = self.create_rate(self.spin_hertz)  # hz
            while True:
                if self.shutdown_request:
                    self.logger.info(f'{self.node_name} {self.shutdown_counter}')
            ...
            self.logger.error(f'{base_rpi.node_name} asy_ros_spin: {error}')
